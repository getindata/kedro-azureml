...
class AzurePipelinesRunner(SequentialRunner):
    ...
    def run(
        ...
        for ds_name, azure_dataset_path in self.data_paths.items():
            if ds_name not in catalog_config:
                if self.pipeline_data_passing:
                    ...
                    dataset_cls = (
                        "kedro_azureml.datasets.runner_dataset.KedroAzureRunnerDataset"
                    )
                ...
                if isinstance(ds, AzureMLPipelineDataset):
                    if (
                        ...
                        catalog_config[ds_name]["root_dir"] = str(
                            Path(azure_dataset_path).parent
                        )
                    else:
        ...
        new_catalog = DataCatalog.from_config(
            catalog_config, credentials, load_versions, save_version
        )

        return super().run(
            pipeline, new_catalog, hook_manager, run_id, only_missing_outputs
        )
