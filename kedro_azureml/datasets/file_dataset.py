...
class AzureMLFileDataSet(PartitionedDataSet):
    ...
    Args:
        ...
        azureml_dataset_save_args: Additional arguments to pass to `AbstractDataset.register` method.
            ...
            please read more here: https://learn.microsoft.com/en-us/python/api/azureml-core/azureml.data.abstract_dataset.abstractdataset?view=azure-ml-py#azureml-data-abstract-dataset-abstractdataset-register
        azureml_dataset_load_args: Additional arguments to pass to `azureml.core.Dataset.get_by_name` method.
            please read more here: https://learn.microsoft.com/en-us/python/api/azureml-core/azureml.core.dataset.dataset?view=azure-ml-py#azureml-core-dataset-dataset-get-by-name
    ...
    Example :
        ...
        Example of python API:
            ...
            >>>     azureml_datastore='my_azureml_datastore_name',  # optional, if not provided, the default datastore will be used
            ...
            >>> # also, it will register a corresponding AzureML file-dataset under the name <my_azureml_file_dataset_name>
    ...
    def __init__(
        ...
        if 'path' in kwargs:
            raise ValueError(f'`path` is not a valid argument for {self.__class__.__name__}')
        ...
        self._azureml_datastore = azureml_datastore or self._workspace.get_default_datastore().name
        ...
        if self._azureml_datastore not in self._workspace.datastores:
            raise ValueError(f'Datastore {self._azureml_datastore} not found in workspace {self._workspace.name}')
    ...
    def _load(self) -> t.Dict[str, t.Callable[[], t.Any]]:
        ...
        dataset = Dataset.get_by_name(self._workspace, name=self._azureml_dataset, **self._azureml_dataset_load_args)
        ...
        override_list_partitions = lambda: partitioned_paths
        try:
