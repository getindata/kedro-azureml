...
Apart from these, ``kedro-azureml`` also adds the ``AzureMLPipelineDataSet`` and ``AzureMLPipelineDistributedDataSet``
which are used to pass data between pipeline nodes when the pipeline is run on Azure ML and the `pipeline_data_passing`
feature is enabled. By default, data is then saved and loaded using the ``PickleDataSet`` as underlying dataset.
Any other underlying dataset can be used instead by adding a ``AzureMLPipelineDataSet`` or ``AzureMLPipelineDistributedDataSet``
to the catalog.
...
.. autoclass:: kedro_azureml.datasets.AzureMLPipelineDistributedDataSet
    :members:
